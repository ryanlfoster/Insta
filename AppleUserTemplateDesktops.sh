...
if [ -r "${RES}/PAYLOAD" ]; then
 ...
 sudo /bin/cp -Rf "${RES}/PAYLOAD/Desktops/" "${ROOT}/${DESKTOP_DIR}"
fi
...
do
 if [ -r "${ROOT}/System/Library/User Template/${USER_TEMPLATE}/Library/Preferences" ]; then
  ...
  sudo /bin/cat > "${ROOT}/System/Library/User Template/${USER_TEMPLATE}/Library/Preferences/com.apple.desktop.plist" << EOPROFILE
